...
module.exports = async ({repository, token, sha, state}) => {
  try {
    console.log(
      `Call GitHub status API https://api.github.com/repos/livingdocsIO/${repository}/statuses/${sha} with state: ${states[state].ghState}`
    )
    return request({
      method: 'POST',
      uri: `https://api.github.com/repos/livingdocsIO/${repository}/statuses/${sha}`,
      body: {
    ...
    console.error(`Failed to call GitHub status API for Repository '${repository}':`, error)
    throw error
  }
}
